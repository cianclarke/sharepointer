    ...
    list : function(listId, cb){
      ...
      if(client.filterFields !== null && client.filterFields.hasOwnProperty('length')) {
        ...
        if(client.filterFields.length === 1) {
          var field = JSON.parse(client.filterFields[0]);
          filterString = field['field'] + '%20eq%20' + "'" + field['value'] + "'";
        }
        else {
          for(i=0;i<client.filterFields.length;i++) {
            var field = JSON.parse(client.filterFields[i]);
            if(i === 0) {
              filterString = '(' + field['field'] + '%20eq%20' + "'" + field['value'] + "')";
            }
            else {
              filterString += '%20and%20(' + field['field'] + '%20eq%20' + "'" + field['value'] + "')";
            }
          }
        }
        filter += filterString;
