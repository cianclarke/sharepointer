  ...
  var doRequest = require('../util/doRequest')(client),
    ...
    crudl = {
      ...
      update: function(listId, listItem, cb) {
        ...
        doRequest({
          url: '/sites/Training2/TestITCARApprovals/_api/contextinfo',
          method: 'POST',
          json: listItem,
          headers: {
            // 'X-RequestDigest': client.baseContext,
...
console.log("digestToekn  --> ",digestToekn);
           ...
           // console.log("client.baseContext in listsItem.js-->", client.baseContext);
            ...
            return doRequest({
              ...
              json: listItem,
              headers: {
                'X-RequestDigest': digestToekn.d?digestToekn.d.GetContextWebInformation?digestToekn.d.GetContextWebInformation.FormDigestValue:"":"",
                'IF-MATCH': '*',
                'content-Type': 'application/json;odata=verbose',
                'X-HTTP-Method': 'MERGE'
              }
