  ...
  var doRequest = require('../util/doRequest')(client),
    ...
    crudl = {
      ...
      byteStream: function(floderName, filename, cb) {
        ...
        currentUser: function(filter,item,cb) {
        var host =  client.BASE_LIST_URL.substring(0,client.BASE_LIST_URL.indexOf('_api/'))+'/_api/web/currentuser?$select='+filter,
        doRequest(host, function(err, items) {
          if (err) {
            return cb(err);
          }
          return cb(null, items);
